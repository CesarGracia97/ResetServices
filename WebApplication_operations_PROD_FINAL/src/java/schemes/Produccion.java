            ...
            if (objeto.contains("Todos")) {
                ...
                if (accion.contains("status")) {
                    ...
                    for (int jb = 0; jb < ruts.length; jb++) {
                        jbossCom = commandResp.getIdProcess(user, host, password, "pgrep -a [j]ava|grep " + ruts[jb] + " |cut -d\" \" -f -1");
                ...
                String jbossAL = "";
                jbossAL = commandResp.getIdProcess(user, host, password, "pgrep -a [j]ava|grep " + cm_instance + " |cut -d\" \" -f -1");
...
//                    String jbossEms = commandResp.getIdProcess("root", host_ems, "cable12", "pgrep -fl [j]ava|grep " + ruta_ems + " |cut -d\" \" -f -1");
                    String tomMob = commandResp.getIdProcess("fsmmobile", "192.168.21.185", "fsmmobile", "pgrep -fl [j]ava|grep " + ruta_toms + "fsmmobile/ |cut -d\" \" -f -1");
                    String tomWww = commandResp.getIdProcess("fsmwww", "192.168.21.184", "fsmwww", "pgrep -fl [j]ava|grep " + ruta_toms + "fsmwww/ |cut -d\" \" -f -1");
                    String tomQue = commandResp.getIdProcess("fsmqueue", "192.168.21.186", "fsmqueue", "pgrep -fl [j]ava|grep " + ruta_toms + "fsmqueue/ |cut -d\" \" -f -1");
                    String tomApi = commandResp.getIdProcess("fsmapi", "192.168.21.186", "fsmap", "pgrep -fl [j]ava|grep " + ruta_toms + "fsmapi/ |cut -d\" \" -f -1");
