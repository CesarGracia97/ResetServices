...
//                if (accion.equals("status")) {
        ...
        if (aplicacion.equals("JBOSS")) {
            ...
            if (objeto.contains("todos")) {
                if (accion.contains("status")) {
                    String jbossC1 = commandResp.getIdProcess("Desarrollo",user, host, password, "pgrep -fa [j]ava|grep desa1 |cut -d' ' -f -1");
                    String jbossCS = commandResp.getIdProcess("Desarrollo",user, host, password, "pgrep -fa [j]ava|grep scheduler |cut -d' ' -f -1");
                    int stat = 0;
                ...
                if (accion.equals("status")) {
                    String jbossAL = "";
                    jbossAL = commandResp.getIdProcess("Desarrollo",user, host, password, "pgrep -fl [j]ava|grep " + objeto + " |cut -d\" \" -f -1");
...
//                        } else {
                        jbossAL = commandResp.getIdProcess("Desarrollo",user, host, password, "pgrep -fl [j]ava|grep " + ruta_toms + user + " |cut -d\" \" -f -1");
//                        }
