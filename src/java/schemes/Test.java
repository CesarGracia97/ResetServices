...
//                if (accion.equals("status")) {
        ...
        if (aplicacion.equals("JBOSS")) {
            ...
            if (objeto.contains("todos")) {
                if (accion.contains("status")) {
                    String jbossC1 = commandResp.getIdProcess("Test",user, host, password, "pgrep -fl [j]ava|grep centest |cut -d' ' -f -1");
...
//                    String[] start = "test".split(",");
                ...
                if (accion.equals("status")) {
                    String jbossAL = "";
                    jbossAL = commandResp.getIdProcess("Test",user, host, password, "ps -aux | grep java|grep qatest |cut -d\" \" -f -1");
//                    jbossAL = commandResp.getIdProcess(user, host, password, "pgrep -fl [j]ava|grep qatest |cut -d' ' -f -1");
        ...
        if (aplicacion.equals("FSM")) {
            ...
            ruta_ems = "/fsmqueue/apache-tomcat-7.0.50/bin/";
            host = "192.168.21.187";
...
//                        } else {
                            ...
                            jbossAL = commandResp.getIdProcess("Test",user, host, password, "pgrep -fl [j]ava|grep " + ruta_toms + user + " |cut -d\" \" -f -1");
//                        }
                        if (jbossAL.length() > 2) {
                            Resultado = "ALL###" + objeto + "--IS RUNNING";
                        } else {
...
//                        }
                    ...
                    if (accion.equals("stop")) {
                        if (user.equals("www")) {
                            user = "rootfsmwww";
                            password = "cable12fsmwww";
                        } else if (user.equals("mobile")) {
                            user = "rootfsmmobile";
                            password = "cable12fsmmobile";
                        } else if (user.equals("api")) {
                            user = "rootfsmapi";
                            password = "cable12fsmapi";
                        } else if (user.equals("queue")) {
                            user = "rootfsmqueue";
                            password = "cable12fsmqueue";
                        } 
